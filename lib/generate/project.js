var Config, fs, Generator, mkdirp, path, Prompt, _slice, Template;
...
Project.prototype.getGenerator = function(generatorName) {
  this.generators.forEach(function(generator) {
    if (generator.modulePath === generatorName) return generator;
  });
};
...
Project.prototype.loadGenerators = function() {
  ...
  function collectGenerators(dirname) {
    ...
    files.forEach(function(file) {
      var basename, name, stat, generator;
...
Project.prototype.loadPrompts = function() {
  var currentPrompt, promptsDir, prompts, promptKey, promptOptions;

  this.prompts = [];

  promptsDir = file.join(this._cwd, this.config.settings.promptsDir);

  function collectPrompts(dirname) {
    var fullpath, files, prompts;

    fullpath = file.join(promptsDir, dirname);
    ...
    if (fs.existsSync(fullpath)) {
      ...
      files.forEach(function(file) {
        var stat, jsonContent, prompt, textContent;

        if (! fs.existsSync(file)) return;

        stat = fs.statSync(file);

        if (fs.isDirectory()) collectPrompts(file.join(dirname, file));

        else {
          textContent = fs.readFileSync(file.join(fullpath, file));

          jsonContent = JSON.parse(textContent);

          prompt = new Prompt(jsonContent);
